...
const command = `docker run --name TypesenseWasteManagement -d -p ${PORT}:8108 -v ${PWD}/typesense-server-data/:/data typesense/typesense:0.22.2 --data-dir /data --api-key=${API_KEY} --listen-port ${PORT}  --enable-cors`;

//`docker container start ${NAME}`
//`docker run --name ${NAME} -d -p ${PORT}:8108 -v ${PWD}/typesense-server-data/:/data typesense/typesense:0.22.2 --data-dir /data --api-key=${API_KEY} --listen-port ${PORT}  --enable-cors`;
